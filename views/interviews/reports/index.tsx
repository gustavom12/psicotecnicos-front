...
const ReportsView = () => {
  ...
  const loadData = async () => {
    try {
      ...
      const interviewsData = Array.isArray(interviewsRes.data?.data)
        ...
        : Array.isArray(interviewsRes.data)
          ? interviewsRes.data
          : [];
  ...
  const generateReport = async () => {
    if (!selectedInterview || !reportTitle.trim()) {
      Notification(
        "Por favor selecciona una entrevista y proporciona un título",
        "error",
      );
      return;
    }
  ...
  const saveReport = async () => {
    try {
      ...
      const errorMessage =
        error?.response?.data?.message || "Error al guardar el informe";
  ...
  const exportToPDF = async () => {
    try {
      ...
      if (!currentReportId) {
        ...
        if (!currentReportId) {
          Notification(
            "Error al guardar el informe antes de exportar",
            "error",
          );
          return;
        }
      ...
      const errorMessage =
        error?.response?.data?.message || "Error al exportar el PDF";
      Notification(errorMessage, "error");
    } finally {
      setGenerating(false);
    }
    ...
    <AuthLayout links={[{ label: "Reports", href: "/reports" }]}>
      <div className=" ">
        ...
        <ButtonGroup className="bg-[#F4F4F5] font-inter text-[14px] text-[#71717A] w-[400px] mt-8 mb-3 h-[36px] rounded-xl">
          ...
          <Button className="bg-white  t  h-[28px]">Informe</Button>
        </ButtonGroup>
            ...
            <div className="bg-white rounded-lg border border-gray-800 p-6 space-y-6">
                ...
                <div className="flex items-center justify-between">
                  <div className="flex items-center space-x-4 text-sm text-gray-600">
                    <div className="flex items-center space-x-2">
                      <button className="font-bold px-2 py-1 hover:bg-gray-100 rounded">
                        B
                      </button>
                      <button className="italic px-2 py-1 hover:bg-gray-100 rounded">
                        I
                      </button>
                      <button className="underline px-2 py-1 hover:bg-gray-100 rounded">
                        U
                      </button>
              ...
              <div className="w-full">
                <Textarea
                  ...
                  onChange={(e) =>
                    setReportContent({ content: e.target.value })
                  }
                  ...
                  classNames={{
                    input:
                      "min-h-[500px] text-gray-800 leading-relaxed resize-none bg-white",
            ...
            <div className="bg-gray-100 rounded-lg p-6 h-full">
              <div className=" rounded-lg p-4 space-y-4 h-full">
                ...
                <p className="text-sm text-gray-700">
                  Tener en consideración las aptitudes resolutivas del
                  entrevistado.
                </p>
                ...
                <div className="flex justify-between items-end mt-10 ">
                  ...
                  <div className="w-24 h-24 bg-white border-2 border-gray-300 rounded-lg flex items-center justify-center shadow-sm">
                    <div
                      className="w-12 h-12 bg-black rounded-full opacity-90"
                      style={{
